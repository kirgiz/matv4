...
export class AddressStockAndSalesUtilityService {
    ...
    search(req?: any): Observable<HttpResponse<AddressStockAndSalesUtility[]>> {
        const options = createRequestOption(req);
        return this.http.get<AddressStockAndSalesUtility[]>(this.resourceSearchUrl, { params: options, observe: 'response' })
            .map((res: HttpResponse<AddressStockAndSalesUtility[]>) => this.convertArrayResponse(res));
    }
