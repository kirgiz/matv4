...
export class CompanyStockAndSalesUtilityService {
    ...
    search(req?: any): Observable<HttpResponse<CompanyStockAndSalesUtility[]>> {
        const options = createRequestOption(req);
        return this.http.get<CompanyStockAndSalesUtility[]>(this.resourceSearchUrl, { params: options, observe: 'response' })
            .map((res: HttpResponse<CompanyStockAndSalesUtility[]>) => this.convertArrayResponse(res));
    }
