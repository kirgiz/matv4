...
export class CivilityStockAndSalesUtilityService {
    ...
    search(req?: any): Observable<HttpResponse<CivilityStockAndSalesUtility[]>> {
        const options = createRequestOption(req);
        return this.http.get<CivilityStockAndSalesUtility[]>(this.resourceSearchUrl, { params: options, observe: 'response' })
            .map((res: HttpResponse<CivilityStockAndSalesUtility[]>) => this.convertArrayResponse(res));
    }
