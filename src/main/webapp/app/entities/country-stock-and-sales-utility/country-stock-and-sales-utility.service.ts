...
export class CountryStockAndSalesUtilityService {
    ...
    search(req?: any): Observable<HttpResponse<CountryStockAndSalesUtility[]>> {
        const options = createRequestOption(req);
        return this.http.get<CountryStockAndSalesUtility[]>(this.resourceSearchUrl, { params: options, observe: 'response' })
            .map((res: HttpResponse<CountryStockAndSalesUtility[]>) => this.convertArrayResponse(res));
    }
