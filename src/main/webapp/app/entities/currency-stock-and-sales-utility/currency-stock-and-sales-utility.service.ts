...
export class CurrencyStockAndSalesUtilityService {
    ...
    search(req?: any): Observable<HttpResponse<CurrencyStockAndSalesUtility[]>> {
        const options = createRequestOption(req);
        return this.http.get<CurrencyStockAndSalesUtility[]>(this.resourceSearchUrl, { params: options, observe: 'response' })
            .map((res: HttpResponse<CurrencyStockAndSalesUtility[]>) => this.convertArrayResponse(res));
    }
