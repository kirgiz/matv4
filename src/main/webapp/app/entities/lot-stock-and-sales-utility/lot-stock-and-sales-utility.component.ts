...
currentAccount: any;
        ...
        if (this.currentSearch) {
            this.lotService.search({
                ...
                sort: this.sort()}).subscribe(
                    (res: HttpResponse<LotStockAndSalesUtility[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/lot-stock-and-sales-utility'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: LotStockAndSalesUtility) {
        return item.id;
    }
