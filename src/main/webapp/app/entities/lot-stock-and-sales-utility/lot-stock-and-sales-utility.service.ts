...
export class LotStockAndSalesUtilityService {
    ...
    search(req?: any): Observable<HttpResponse<LotStockAndSalesUtility[]>> {
        const options = createRequestOption(req);
        return this.http.get<LotStockAndSalesUtility[]>(this.resourceSearchUrl, { params: options, observe: 'response' })
            .map((res: HttpResponse<LotStockAndSalesUtility[]>) => this.convertArrayResponse(res));
    }
