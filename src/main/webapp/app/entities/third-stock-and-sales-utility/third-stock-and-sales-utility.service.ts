...
export class ThirdStockAndSalesUtilityService {
    ...
    search(req?: any): Observable<HttpResponse<ThirdStockAndSalesUtility[]>> {
        const options = createRequestOption(req);
        return this.http.get<ThirdStockAndSalesUtility[]>(this.resourceSearchUrl, { params: options, observe: 'response' })
            .map((res: HttpResponse<ThirdStockAndSalesUtility[]>) => this.convertArrayResponse(res));
    }
