...
export class ForexratesStockAndSalesUtilityService {
    ...
    search(req?: any): Observable<HttpResponse<ForexratesStockAndSalesUtility[]>> {
        const options = createRequestOption(req);
        return this.http.get<ForexratesStockAndSalesUtility[]>(this.resourceSearchUrl, { params: options, observe: 'response' })
            .map((res: HttpResponse<ForexratesStockAndSalesUtility[]>) => this.convertArrayResponse(res));
    }
